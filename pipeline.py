...
###############################################################################
SUMMARIZE_PROMPT = """你是一位计算化学/电催化领域的研究助理。阅读以下论文全文（或摘要），用 `- **字段名**：值` 格式输出以下字段（每行一个，不要使用 ### 子标题）：

- **核心问题**：一句话总结本文试图解决的科学问题
- **动机与背景**：现有方法痛点及为什么值得解决（2-3句话）
- **方法核心**：方法名称和技术主干（1-2句话，突出创新）
- **关键实验与结果**：主要体系、基线方法、1-2个关键数值结果
- **创新点**：2-3点，明确与已有工作的差异
- **局限性**：本文的不足或未解决的问题
- **对你研究的启发**：可迁移的思路或方法论启发
- **与你研究的相关度**：高/中/低

论文："""

BRIEF_PROMPT = """你是一位计算化学/电催化领域的研究助理。阅读以下论文摘要，用 `- **字段名**：值` 格式输出以下字段（每行一个，不要多余内容）：
...
###############################################################################
HIGHLIGHTS_PROMPT = """你是一位计算化学/电催化领域的研究助理。今天阅读了以下论文，请从研究者的角度分析。使用以下固定格式输出，不要添加多余内容：

**今日亮点**
- **最值得精读**：[论文编号] 标题 — 为什么最有价值（1-2句）
- **可能冲突的研究**：[论文编号] 标题 — 可能与你研究论点冲突的方面（1句）
- **值得追踪的团队**：作者/团队名 — 值得关注的原因（1句）
- **重要趋势**：今天论文体现的值得注意的趋势或发现（1句）

**关键差距**
- **Gap 1**：描述一个共同暴露的未解决问题或方法论局限（1-2句）
- **Gap 2**：描述另一个未解决的问题（1-2句）
- **未来方向**：建议的未来探索方向（1-2句）
...
def generate_highlights_gaps(papers_with_summaries):
    ...
    try:
        ...
        parts = result.split("**关键差距**", 1)
        highlights = parts[0].replace("**今日亮点**", "").strip()
        gaps = "**关键差距**\n" + parts[1].strip() if len(parts) > 1 else "**关键差距**\n- *无分析*"
